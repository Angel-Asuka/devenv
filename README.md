...
如果环境尚不存在，工具会自动基于 `ubuntu:26.04` 创建 `devenv:python-dev`。当前目录会以相同的绝对路径挂载进容器，并作为容器工作目录；退出 shell 后临时容器会被删除，项目文件仍保留在宿主机上。
...
| `devenv run <环境名> [Docker 参数...]` | 启动临时开发容器。环境不存在时会自动创建。 |
| `devenv maint <环境名>` | 进入维护容器；退出时自动保存变更。环境不存在时会自动创建。 |
...
项目源码通过挂载保存在宿主机；只有维护模式中对容器文件系统的修改（例如安装工具链）会保存到环境镜像。需要移除环境时执行：
