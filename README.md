...
如果环境尚不存在，工具会自动基于 `ubuntu:26.04` 创建 `devenv:python-dev`。启动时会在容器内创建（或复用）与当前宿主用户相同 UID/GID 的账号，并以该账号运行 shell。当前目录会以相同的绝对路径挂载进容器，并作为容器工作目录；退出 shell 后临时容器会被删除，项目文件仍保留在宿主机上。因此在挂载目录中新建的文件不会归属为 root。
