...
首次运行每个环境时，工具会将 `sudo` 安装并保存到该环境镜像，同时为当前容器用户配置免密 sudo。进入环境后可使用：

```bash
sudo <命令>
sudo -i        # 进入 root shell
sudo su -      # 等效的 root shell 方式
```

`su` 命令也可用；通常建议使用 `sudo -i` 进行提权，因为 root 账户默认没有可供交互输入的密码。首次准备 sudo 需要能够访问 Ubuntu 软件源。
